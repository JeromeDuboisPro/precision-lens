...
def print_detailed_analysis(results_by_precision):
    ...
    for precision in ["FP64", "FP32", "FP16", "FP8"]:
        ...
        for result in sorted(
            results, key=lambda x: (x["matrix_size"], x["condition_number"])
        ):
            ...
            if converged:
                print(
                    f"  cond={cond:>4}, n={n:>4}: ✓ Converged at iteration {conv_iter:>4} (final error: {final_err:.2e})"
                )
            else:
                print(
...
def print_summary_table(results_by_precision):
    ...
    print("=" * 80)
    print(
        f"{'Precision':<10} {'Cond':<10} {'Matrix Size':<12} {'Converged':<12} {'Iteration':<15}"
    )
    print("-" * 80)
